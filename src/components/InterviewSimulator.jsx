...
// Builds the system prompt injecting the job description; forces structured output with delimiters
...
export default function InterviewSimulator() {
  // Active screen: "home" | "setup" | "interview" | "history"
  const [screen, setScreen] = useState("home");
  // Raw job description text pasted by the user
  const [jobDescription, setJobDescription] = useState("");
  // Optional display name for the role (used in saved simulation cards)
  const [jobTitle, setJobTitle] = useState("");
  // Ordered list of chat messages for the current session
  const [messages, setMessages] = useState([]);
  // Current value of the answer textarea
  const [input, setInput] = useState("");
  // True while waiting for the model response
  const [loading, setLoading] = useState(false);
  // Per-question scores collected during the session
  const [scores, setScores] = useState([]);
  // How many questions have been asked so far
  const [questionCount, setQuestionCount] = useState(0);
  // True once QUESTIONS_COUNT answers have been submitted
  const [finished, setFinished] = useState(false);
  // All saved simulations loaded from storage
  const [simulations, setSimulations] = useState([]);
  // Simulation selected in the history sidebar for detail view
  const [selectedSim, setSelectedSim] = useState(null);
  // Ref to the invisible div at the bottom of the chat, used to auto-scroll
  ...
  // Fetches all sessions from the server; newest first (server stores oldest first)
  ...
  // Posts the new simulation to the server and prepends the saved object (with server-generated id) to local state
  ...
  // Deletes the session on the server, then removes it from local state
  ...
  // Parses the model's delimited response into { feedback, score, question }
  ...
  // Resets all session state and fetches the first question from the model
  ...
  // Submits the candidate's answer, appends the model reply, and saves on last question
  const sendAnswer = async () => {
    ...
    // Flag the last turn so the model produces a closing summary
    const isLast = questionCount >= QUESTIONS_COUNT;
    try {
      // Convert internal message format to OpenAI roles; interviewer messages use the delimiter prefix
      ...
      const finalMessages = [...newMessages, { role: "interviewer", text: parsed.question || data.choices[0].message.content, feedback: parsed.feedback, score: newScore }];
      setMessages(finalMessages);
      if (isLast) {
        setFinished(true);
        // Compute average; "N/A" only if the model never returned a numeric score
  ...
  // Live average across scores collected so far; null before the first scored answer
  const avgScore = scores.length > 0 ? (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1) : null;
  // Maps a numeric score to a green / yellow / red hex color
  ...
  // Header dot color signals the current mode at a glance
  const dotColor = screen === "interview" && !finished ? "#4ade80" : screen === "history" ? "#facc15" : "#6366f1";

  // Renders a list of messages; shared by the live interview view and the history detail panel
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#e2e8f0", display: "flex", flexDirection: "column" }}>
      ...
      <div
        ...
        {/* Logo — navigates home only when not mid-interview */}
      ...
      {screen === "home" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 40, padding: "48px 20px" }}>
          ...
          {/* Quick-access card for the most recent simulation */}
          {simulations.length > 0 && (
            <div style={{ width: "100%", maxWidth: 540 }}>
              ...
              <div className="sim-card" onClick={() => { setSelectedSim(simulations[0]); setScreen("history"); }}>
                ...
                {/* Mini bar chart: one colored bar per question score */}
      ...
      {screen === "interview" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", maxWidth: 820, width: "100%", margin: "0 auto", padding: "0 20px", minHeight: 0 }}>
          <div style={{ flex: 1, overflowY: "auto", padding: "24px 0", display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {/* Typing indicator shown while awaiting model response */}
            {loading && (
            ...
            {/* End-of-session summary card with per-question scores */}
            {finished && avgScore && (
            ...
            {/* Invisible anchor used by the scroll-to-bottom effect */}
            <div ref={bottomRef} />
          </div>
          {!finished && (
      ...
      {screen === "history" && (
        <div style={{ flex: 1, display: "flex", maxWidth: 940, width: "100%", margin: "0 auto", padding: "28px 20px", gap: 24, minHeight: 0, overflow: "hidden" }}>
          {/* Sidebar: scrollable list of saved simulation cards */}
          ...
          {/* Detail panel: full transcript of the selected simulation */}
